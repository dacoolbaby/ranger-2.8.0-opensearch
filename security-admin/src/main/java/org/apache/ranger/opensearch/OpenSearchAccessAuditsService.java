...
public class OpenSearchAccessAuditsService extends org.apache.ranger.AccessAuditsService {
	...
	public VXAccessAuditList searchXAccessAudits(SearchCriteria searchCriteria) {
		...
		try {
			...
			LOGGER.warn(String.format("OpenSearch query failed: %s", e.getMessage()));
			throw restErrorUtil.createRESTException(
		...
		try {
			...
			LOGGER.warn(String.format("OpenSearch fetch failed: %s", e.getMessage()));
			throw restErrorUtil.createRESTException(
					"Error querying search engine",
					MessageEnums.ERROR_SYSTEM);
		}
				...
				if (!hiveQueryVisibility && isHive) {
						...
						if ("grant".equalsIgnoreCase(accessType)
								...
								if (requestData != null) {
									...
									LOGGER.warn(
											"Error in request data of audit from openSearch. AuditData: "
